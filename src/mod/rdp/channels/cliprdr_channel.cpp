...
::read(std::size_t n)
{
    ...
    if (this->data_len <= file_content_memory_max) {
        ...
        if (this->requested_buffer.remaining() < n) {
            size_t const used = this->requested_buffer.remaining();
            auto* const p = this->requested_buffer.buf.get();
            memmove(p, this->requested_buffer.current(), used);
            auto const len = std::min(requested_len_max - used, this->data_len - this->data_pos);
            this->requested_buffer.capacity = used + len;
            this->requested_buffer.pos = 0;

            if (ssize_t(len) != ::read(this->fd.fd(), p + used, len)) {
                const int errnum = errno;
                LOG(LOG_ERR,
...
void ClipboardVirtualChannel::process_server_message(uint32_t total_length,
    uint32_t flags, bytes_view chunk_data, RDPECLIP::CliprdrHeader const* decoded_header)
{
    (void)decoded_header;
    this->process_server_message(total_length, flags, chunk_data);
}
...
{
    (void)decoded_header;
    this->process_client_message(total_length, flags, chunk_data);
}
