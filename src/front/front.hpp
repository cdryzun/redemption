        ...
        ::save_persistent_disk_bitmap_cache(
            ...
            [this](const Error & error){
                if (error.errnum == ENOSPC) {
                    ...
                    this->acl_report.acl_report(AclReport::file_system_full());
                }
            },
...
public:
    void set_keyboard_indicators(kbdtypes::KeyLocks key_locks) override
    {
        ...
        // Payload
        stream.get_data_stream().out_uint16_le(0);
        stream.get_data_stream().out_uint16_le(safe_int(key_locks));

        ::send_share_data_ex(
            this->trans, PDUTYPE2_SET_KEYBOARD_INDICATORS, false,
            this->mppc_enc.get(), this->share_id, this->encryptionLevel,
            this->encrypt, this->userid, stream,
            bool(this->verbose & (Verbose::basic_trace | Verbose::sec_decrypted)));
    }
