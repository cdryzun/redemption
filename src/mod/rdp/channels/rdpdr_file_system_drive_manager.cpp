...
public:
    ManagedFileSystemObject(AsynchronousTaskContainer& async_task_container)
    : async_task_container(async_task_container)
    {}
    ...
    virtual void process_server_drive_control_request(
        ...
        uint32_t out_flags = CHANNELS::CHANNEL_FLAG_FIRST | CHANNELS::CHANNEL_FLAG_LAST;
          ...
          this->async_task_container.add(std::make_unique<RdpdrSendDriveIOResponseTask>(
    ...
    virtual void ProcessServerDriveSetInformationRequest(
        ...
        switch (server_drive_set_information_request.FsInformationClass())
        {
            ...
            case rdpdr::FileAllocationInformation:
            {
                ...
                LOG_IF(bool(verbose & RDPVerbose::fsdrvmgr), LOG_INFO,
                    "ManagedFileSystemObject::ProcessServerDriveSetInformationRequest: "
                    "AllocationSize=%" PRId64, AllocationSize);
...
private:
    static void SendClientDriveIoResponse(
        rdpdr::DeviceIORequest const & device_io_request,
        const char * message,
        erref::NTSTATUS IoStatus,
        VirtualChannelDataSender & to_server_sender,
        ...
        RDPVerbose verbose)
    {
        ...
        async_task_container.add(std::make_unique<RdpdrSendDriveIOResponseTask>(
            out_flags, out_stream.get_data(), out_stream.get_offset(), to_server_sender,
            verbose));
    }

    void SendClientDriveSetInformationResponse(
        rdpdr::DeviceIORequest const & device_io_request,
        const char * message,
        erref::NTSTATUS IoStatus,
        uint32_t Length,
...
public:
    static void SendClientDriveIoUnsuccessfulResponse(
        rdpdr::DeviceIORequest const & device_io_request,
        const char * message,
        VirtualChannelDataSender & to_server_sender,
        AsynchronousTaskContainer& async_task_container,
...
public:
    ManagedDirectory(AsynchronousTaskContainer& async_task_container)
    : ManagedFileSystemObject(async_task_container)
    {
    ...
    void ProcessServerDriveQueryDirectoryRequest(
        ...
        LOG_IF(bool(verbose & RDPVerbose::fsdrvmgr), LOG_INFO,
            "ManagedDirectory::ProcessServerDriveQueryDirectoryRequest: full_path=\"%s\" pattern=\"%s\"",
            this->full_path, this->pattern);

        dirent * entry = nullptr;
        while ((entry = ::readdir(this->dir))) {
            auto filename = zstring_view::from_null_terminated(entry->d_name);
...
public:
    ManagedFile(AsynchronousTaskContainer& async_task_container)
    : ManagedFileSystemObject(async_task_container)
    {
    ...
    void process_server_drive_read_request(
        ...
        this->async_task_container.add(std::make_unique<RdpdrDriveReadTask>(
...
public:
    void ProcessServerDriveWriteRequest(
        ...
        if (first_chunk) {
            ...
            LOG_IF(bool(verbose & RDPVerbose::fsdrvmgr), LOG_INFO,
                "ManagedFile::ProcessServerDriveWriteRequest(): Length=%u Offset=%" PRIu64,
                Length, Offset);
        }
...
struct FileSystemDriveManager::managed_file_system_object_type
{
    uint32_t file_id;
    std::unique_ptr<ManagedFileSystemObject> object;
};

FileSystemDriveManager::FileSystemDriveManager(
    AsynchronousTaskContainer& async_task_container, RDPVerbose verbose)
: async_task_container(async_task_container)
, verbose(verbose)
{}

FileSystemDriveManager::~FileSystemDriveManager() = default;

void FileSystemDriveManager::announce_drive(
    ...
    bool read_only)
{
    ...
    std::string directory_drive_path = str_concat(
        directory_drive_path_view,
        directory_drive_path_view.empty() ? ""_av : "/"_av,
        drive_name.name()
    );

    if (((::stat(directory_drive_path.c_str(), &sb) == 0) && S_ISDIR(sb.st_mode))) {
        LOG_IF(bool(verbose & RDPVerbose::fsdrvmgr), LOG_INFO,
            "FileSystemDriveManager::enable_drive: directory_path=\"%s\"",
...
[[nodiscard]] FileSystemDriveManager::managed_drive_collection_type::const_iterator
FileSystemDriveManager::find_drive_by_id(uint32_t DeviceId) const
{
    return std::find_if(
    ...
    VirtualChannelDataSender & to_server_sender)
{
    ...
    managed_file_system_object->process_server_create_drive_request(
            ...
            is_session_probe_image_flag, verbose);
    if (drive_created) {
...
void FileSystemDriveManager::process_device_IO_request(
    rdpdr::DeviceIORequest const & device_io_request,
    bool first_chunk,
    InStream & in_stream,
    VirtualChannelDataSender & to_server_sender)
    ...
    switch (device_io_request.MajorFunction()) {
        case rdpdr::IRP_MJ_CREATE:
            ...
            this->process_server_create_drive_request(device_io_request,
                path, drive_access_mode, in_stream, to_server_sender);
        break;
        ...
        case rdpdr::IRP_MJ_READ:
...

            {
                ...
                file_iter->object->process_server_drive_read_request(
                    ...
                    in_stream, to_server_sender, verbose);
            }
        break;
        ...
        case rdpdr::IRP_MJ_WRITE:
            ...
            file_iter->object->ProcessServerDriveWriteRequest(
                ...
                first_chunk, in_stream, to_server_sender, verbose);
        break;
        ...
        case rdpdr::IRP_MJ_DEVICE_CONTROL:
...

            {
                ...
                file_iter->object->process_server_drive_control_request(
                    ...
                    path.c_str(), in_stream, to_server_sender, verbose);
            }
        break;
        ...
        case rdpdr::IRP_MJ_SET_INFORMATION:
...

            {
                ...
                file_iter->object->ProcessServerDriveSetInformationRequest(
                    ...
                    drive_access_mode, in_stream, to_server_sender, verbose);
            }
        break;
...
void FileSystemDriveManager::remove_session_probe_drive()
{
    if (this->session_probe_drive_id == INVALID_MANAGED_DRIVE_ID) {
        return;
    }
...
void FileSystemDriveManager::disable_session_probe_drive(VirtualChannelDataSender & to_server_sender)
{
    if (this->session_probe_drive_id == INVALID_MANAGED_DRIVE_ID) {
        return;
    }
