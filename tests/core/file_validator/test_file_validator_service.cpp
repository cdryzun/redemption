...
RED_AUTO_TEST_CASE(file_validatorSendFile)
{
    ...
    auto filename = FIXTURES_PATH "/test_infile.txt"_zv;
    auto content = RED_REQUIRE_GET_FILE_CONTENTS(filename);

    FileValidatorId id = file_validator.open_file(filename, "clamav"_av);
    ...
    RED_CHECK(file_validator.open_file(filename, "clamav"_av) == FileValidatorId(2));
}
...
RED_AUTO_TEST_CASE(file_validatorSendText)
{
    ...
    FileValidatorId id = file_validator.open_text(0x12345678, "clamav"_av);
    RED_CHECK(id == FileValidatorId(1));
    ...
    RED_CHECK(file_validator.open_file("filename"_av, "clamav"_av) == FileValidatorId(2));
}
...
RED_AUTO_TEST_CASE(file_validatorSendUnicode)
{
    ...
    FileValidatorId id = file_validator.open_unicode("clamav"_av);
    RED_CHECK(id == FileValidatorId(1));
